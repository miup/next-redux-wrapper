...
const Page: FC<ReduxWrapperPageProps<State>> = ({store}) => (
...
const ConnectedPage = connect((state: State) => state)({tick} => {
    return (
        <div>{tick}</div>
    );
});
...
class Page extends Component<ReduxWrapperPageProps<State>> {
    ...
    render() {
        const {tick} = this.props.store.getState();
        return (
            <Provider store={store}>
                <ConnectedPage />
            </Provider>
...
5. App that used `getInitialStore` need to have App `getInitialStore` wrapped with `wrapper.getInitialStore`, Page's `getInitialStore` can remain unwrapped then

6. If you haven't used App then Pages' `getInitialStore` do need to be wrapped with `wrapper.getInitialStore`
